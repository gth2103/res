    ...
    items.forEach(function(item){
        ...
        if(item != null && item.item_id != null){
            ...
            if (is_current_user_item){
                ...
                div = '<div class="item col-3 my-5 text-center"><img class="item-thumbnail mb-4" src="' + item.image + '"><br><div class="item_title overflow-auto">' + item.title + '</div><p class="buy_sell_price my-2"> $' + item.price + '</p><button id="' + item.item_id + '" class="view btn btn-outline-secondary my-2 text-center" type="submit">View</button></div>'
            }
            else {

                div = '<div class="item col-3 my-5 text-center"><img class="item-thumbnail mb-4" src="' + item.image + '"><br><div class="item_title overflow-auto">' + item.title + '</div><p class="buy_sell_price my-2"> $' + item.price + '</p><button id="' + item.item_id + '" class="view btn btn-outline-secondary my-2 ml-4 float-left" type="submit">View</button><button id="' + item.item_id + '" class="add_to_cart btn btn-outline-info my-2 mr-4 float-right" type="submit">Add to cart</button></div>'
            }
            $('#items').append(div)
        }
    })
